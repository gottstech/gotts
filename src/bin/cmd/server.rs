...
/// Gotts server commands processing
...
/// stopping the Gotts blockchain server. Processes all the command line
/// arguments to build a proper configuration and runs Gotts with that
/// configuration.
pub fn server_command(
	...
	if let Some(a) = server_args {
		match a.subcommand() {
			...
			("", _) => {
				println!("Subcommand required, use 'gotts help server' for details");
			}
			(cmd, _) => {
				println!(":: {:?}", server_args);
				panic!(
					"Unknown server command '{}', use 'gotts help server' for details",
					cmd
				);
			}
		}
