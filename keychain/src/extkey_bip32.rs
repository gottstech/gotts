...
//! Modified from above to integrate into gotts and allow for different
...
/// Gotts uses blake2 everywhere but the spec calls for SHA512/Ripemd160, so allow
...
#[derive(Clone, Debug)]
pub struct BIP32GottsHasher {
	is_floo: bool,
	hmac_sha512: Hmac<Sha512>,
}

impl BIP32GottsHasher {
	/// New empty hasher
	pub fn new(is_floo: bool) -> BIP32GottsHasher {
		BIP32GottsHasher {
			is_floo: is_floo,
...
impl BIP32Hasher for BIP32GottsHasher {
	fn network_priv(&self) -> [u8; 4] {
		match self.is_floo {
...
impl ExtendedPrivKey {
	...
	pub fn new_master<H>(hasher: &mut H, seed: &[u8]) -> Result<ExtendedPrivKey, Error>
	where
		H: BIP32Hasher,
	{
...
impl ExtendedPubKey {
	...
	where
		H: BIP32Hasher,
	{
		let (sk, chain_code) = self.ckd_pub_tweak(hasher, i)?;
...
mod tests {
	...
	fn test_path(
		...
		let mut sk = ExtendedPrivKey::new_master(&mut h, seed).unwrap();
