...
impl From<gotts_store::Error> for Error {
	fn from(e: gotts_store::Error) -> Error {
		Error::Store(e)
	}
}
...
/// included in gotts-server.toml, but we don't want them to ever return none
impl P2PConfig {
	/// return ban window
...
pub trait ChainAdapter: Sync + Send {
	...
	/// Get the Gotts specific tmp dir
